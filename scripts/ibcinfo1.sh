...
# fetch chain ID
chainId=$(fetch_data "${restAddress}/cosmos/base/tendermint/v1beta1/node_info" | jq -r '.default_node_info.network')
...
counterpartyChainId=$(fetch_data "${restAddress}/ibc/core/channel/v1/channels/${channelId}/ports/${portId}/client_state" | jq -r '.identified_client_state.client_state.chain_id')

# check for errors in outputs
if ! [[ $clientId =~ ^07-tendermint-[0-9]{1,5}$ ]]; then
  echo -e "${RED}Unexpected format in fetched client ID. Received: $clientId${NC}"
fi

# print chain IDs
echo -e "${GREEN}Chain ID: ${chainId}${NC}"
echo -e "${GREEN}Counterparty Chain ID: ${counterpartyChainId}${NC}"
